 ...
 * <li>It demultiplexes incoming connections based on a {@link StartupAnnounceMessage} that is sent
 *     over the socket.
 * </ol>
 *
 ...
 *     errors while accepting connections or reading from connections.
 * </ul>
 *
 ...
 *
 * <ul>
 * <li>If you make getConnection a blocking method that calls accept until it finds the connection
 *     with its id, then there is no way to deal with connections that never arrive. For example, if
 *     the worker crashes before connecting then the thread calling accept will block forever
 *     waiting for it. The only way to unblock a thread stuck on accept() is to close the socket
 *     (this holds for ServerSocketChannels and normal ServerSockets), but we cannot do that in this
 *     case because the socket is a shared resource.
 * <li>If you make getConnection a non-blocking, polling based method then you expose yourself to
 ...
 *     its output streams and then calls Socket.connect, and no printing to stdout or stderr occurs
 *     while connecting. Then if the runner polls, but misses the connection and then tries to read
